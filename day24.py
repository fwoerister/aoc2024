...
def fix_circuit(gates, pos, swaps):
    if len(swaps) > 4:
        return None
    ...
    if pos == 0:
        ...
        return fix_circuit(gates, pos + 1, swaps)
    ...
    if not z:
        for potential_operand in find_potential_operand(gates, 'XOR', zp[0]):
            ...
            if final_swaps := fix_circuit(new_gates, pos, new_swaps):
                return final_swaps
        ...
        return fix_circuit(new_gates, pos, new_swaps)
...
if '__main__' == __name__:
    ...
    involved_gates = set()
    for swap in fix_circuit(gates, 0, []):
